...
export const POST_IMAGE_MAX_BYTES = 500 * 1024;
...
function normalizeCompressedFile(originalFile: File, compressedFile: File) {
...
async function compressImage(file: File, options: Options) {
...
export function isPostImageWithinLimit(file: File) {
	return file.size <= POST_IMAGE_MAX_BYTES;
}

export function isAvatarImageWithinLimit(file: File) {
	return file.size <= AVATAR_IMAGE_MAX_BYTES;
}

export function compressPostImage(file: File) {
...
export function compressAvatarImage(file: File) {
	return compressImage(file, {
